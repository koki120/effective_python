...
# 組み込み型listのsortメソッドはほとんどの組み込み型で提供されている。
...
# classは比較を定義しないとsortを呼び出すことはできない。
class Tool:
...
# pythonのソートは安定ソートアルゴリズムが使われているので、同じ値の場合は順番は変わらない。
# この性質を利用して、sort(key=lambda x:(x.weight, -x.name))を下のように実現できる。-x.nameは使えないので、注意。
...
tools2.sort(key=lambda x: x.weight)
